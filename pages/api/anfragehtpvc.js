  ...
  try {
    ...
    if (mitSeitenteil) {
      ...
      if (breiteSeitenteil + breiteTuerfluegelBeiSeitenteil !== gesamtBreiteMm) {
        return res.status(400).json({ error: "Die Summe von Seitenteil und Türflügel stimmt nicht mit der Gesamtbreite überein." });
      }
    ...
    if (mit2Seitenteilen) {
      ...
      if (
        ...
        return res.status(400).json({ error: "Die Summe der drei Breiten stimmt nicht mit der Gesamtbreite überein." });
      }
    }
    ...
    if (mitOberlicht) {
      ...
      if (hoeheTuerfluegel + hoeheOberlicht !== gesamtHoeheMm) {
        return res.status(400).json({ error: "Die Summe von Türflügel und Oberlicht stimmt nicht mit der Gesamthöhe überein." });
      }
    }
    ...
    const payload = {
      ...
      aussen: String(b.aussen || "Stoßgriff").trim(),
